...
const SignUpScreen = ({ navigation }) => {
    ...
    const handleSignUp = async () => {
        ...
        try {
            const response = await fetch('https://angry-games-trade.loca.lt/auth/register', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                Alert.alert("Registration Error", errorData.detail || "Could not register user. Email might be already in use.");
                return;
            }

            // Immediately login after registration
            const loginFormData = `username=${encodeURIComponent(email.toLowerCase().trim())}&password=${encodeURIComponent(password)}`;
            const loginResponse = await fetch('http://192.168.1.11:8000/auth/login', {
                method: 'POST',
                headers: {
            ...
            if (loginResponse.ok) {
                ...
                dispatch(setUser({ name, email: email.toLowerCase().trim() })); 
            }
